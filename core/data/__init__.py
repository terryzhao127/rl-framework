...
class DataCollection:
    ...
    def push(self, state, action, action_prob, reward, next_state, done):
        self.buffer.append([state, action, action_prob, reward])
        ...
        if self.done or len(self.buffer) == self.size:
            ...
            actions, action_probs, rewards = [arr2bytes(np.array(x).reshape(-1)) for x in items[1:]]
            next_state = arr2bytes(self.next_state)

            data = Data(states=states, actions=actions, action_probs=action_probs,
...
def parse_data(data_string):
    ...
    items = [data.states, data.actions, data.action_probs, data.rewards, data.next_state]
